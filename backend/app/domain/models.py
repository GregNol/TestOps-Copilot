...
class RedactRequest(BaseModel):
    original_content: str = Field(..., description="Исходный текст (тест-план или код)")
    edit_instructions: str = Field(..., description="Что нужно изменить/добавить")

class GenerateAutoTestsRequest(GenerateTestsRequest):
    approved_test_plan: str = Field(..., description="Утвержденный ручной тест-план")

class OptimizationRequest(BaseModel):
    modules: str = Field(..., description="Список модулей приложения")
    test_cases: str = Field(..., description="Список текущих тест-кейсов (текст или код)")

class ReviewRequest(BaseModel):
    code_snippet: str = Field(..., description="Код автотестов для проверки")
    rules: str = Field("Стандартные правила Allure и Pytest", description="Доп. правила")
